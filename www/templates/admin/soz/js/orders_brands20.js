...
//------------------------------------------------------------------------------------------------------
					...
					$('.send_mail').click(function(){
						...
						var price=0;
							$('#but22_tab'+rel+' .gr_tab').each(function(){
								...
								if(rel2==47){
									tabPrice=parseInt($(this).find('td:eq(7)').attr('rel-price-opt'));
									PriceOrOpt=2;
								}
					...
					$('.tab_brenda').each(function(){
						...
						$(this).find('.gr_tab').each(function(){
							...
							if(sel!=2){
								gr_set_count+=parseInt($(this).find('td').eq(5).text());
								gr_set_sum+=parseInt($(this).find('td').eq(8).text());
							}
						});
						if(brendaId==48){
